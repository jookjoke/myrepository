define(function(require,exports,module){
	...
	baseObj.loadImg(flyBird,function(obj){
		bird = flappyBird.Bird(obj["birds"],10,10,52,48);
		var skyArr = [];
		for (var i = 0; i < 2; i++) {
			sky = flappyBird.Sky(obj["sky"],obj["sky"].width * i);
			skyArr.push(sky);
		}
		...
		for (var i = 0; i < length; i++) {
			land = flappyBird.Land(obj["land"],obj["land"].width * i,canvas.height - obj["land"].height);
			landArr.push(land);
		}

		var pipeArr = [];
		for (var i = 0; i < 5; i++) {
			pipe = flappyBird.Pipe(obj["pipeDown"],obj["pipeUp"],200+((obj["pipeDown"].width - 2)* 4 * i));
			pipeArr.push(pipe);	
		}
